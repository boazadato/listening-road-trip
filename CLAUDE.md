...
**Implementation plan:** [`docs/superpowers/plans/2026-05-29-listening-road-trip.md`](docs/superpowers/plans/2026-05-29-listening-road-trip.md) — 17 tasks, each with exact file paths, code, and commit steps. The plan is the single source of truth for the task list (no separate issue tracker). **Task status lives in the plan index's [Tasks](docs/superpowers/plans/2026-05-29-listening-road-trip.md#tasks) list** (⬜ todo · 🔄 in progress · ✅ done) — not in `git log`. Identify the next task as the lowest-numbered one still marked ⬜, and flip its marker to ✅ in the same commit that completes it.
